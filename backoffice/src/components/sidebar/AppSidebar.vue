      ...
      <VaCollapse v-for="(route, index) in navigationRoutes.routes" :key="index">
        <template #header="{ value: isCollapsed }">
          <VaSidebarItem
            ...
            :aria-label="`${route.children ? 'Abrir categoria ' : 'Ir para'} ${route.displayName}`"
            role="button"
            hover-opacity="0.10"
          >
        ...
        <template #body>
          <div v-for="(childRoute, index2) in route.children" :key="index2">
            <VaSidebarItem
              ...
              :aria-label="`Ir para ${childRoute.displayName}`"
              hover-opacity="0.10"
            >
...
const routeHasActiveChild = (section: INavigationRoute) => {
  if (!section.children) {
    return route.path.endsWith(`${section.name}`) || route.name === section.name
  }
  return section.children.some(({ name }) => route.path.endsWith(`${name}`) || route.name === name)
}
