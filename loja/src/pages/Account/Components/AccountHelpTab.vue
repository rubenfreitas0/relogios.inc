...
<template>
	<div class="space-y-8">
		...
		<div class="grid gap-6 md:grid-cols-2">
			...
			<div class="rounded-xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
				<div class="mb-4 flex items-center gap-3">
					<div class="flex h-10 w-10 items-center justify-center rounded-lg bg-[#FFC700]/10 text-[#FFC700]">
						<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 00-2-2H6a2 2 0 00-2 2v16l4-2 4 2 4-2 4 2z" />
						</svg>
					</div>
					<h3 class="text-sm font-bold uppercase tracking-wider text-white">Devoluções e Reembolsos</h3>
				</div>
				<p class="text-xs leading-relaxed text-white/50">
					Garantimos o direito de devolução de <strong>14 dias</strong> após a receção dos artigos. O produto deve ser devolvido na sua embalagem original, sem sinais de uso, completo e acompanhado da respetiva fatura. O reembolso é feito no prazo de 14 dias pelo mesmo método de pagamento.
				</p>
			</div>
			...
			<div class="rounded-xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
				<div class="mb-4 flex items-center gap-3">
					...
					<h3 class="text-sm font-bold uppercase tracking-wider text-white">Garantia de 3 Anos</h3>
				</div>
				<p class="text-xs leading-relaxed text-white/50">
					Todos os nossos relógios dispõem de uma garantia legal de <strong>3 anos</strong> contra defeitos de fabrico e anomalias do mecanismo, contados a partir da data de entrega, em conformidade com o Decreto-Lei n.º 84/2021 em Portugal.
				</p>
			</div>
		</div>
		...
		<!-- FAQs -->
		<div class="rounded-xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
			<h3 class="mb-6 text-base font-bold uppercase tracking-wider text-[#FFC700]">Perguntas Frequentes (FAQ)</h3>
			<div class="divide-y divide-white/10">
				<div v-for="(faq, index) in faqs" :key="index" class="py-4 first:pt-0 last:pb-0">
					<button
		...
		<div class="rounded-xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
			<h3 class="mb-2 text-base font-bold uppercase tracking-wider text-[#FFC700]">Contactar o Apoio ao Cliente</h3>
			<p class="mb-6 text-xs text-white/40">Precisa de ajuda com uma devolução, reembolso ou garantia? Envie-nos uma mensagem direta.</p>

			<form @submit.prevent="handleContactSubmit" class="space-y-4">
				<div class="grid gap-4 sm:grid-cols-2">
					<div>
						<label class="mb-1.5 block text-[0.65rem] font-bold uppercase tracking-[0.15em] text-white/45">
							Assunto do Contacto
						</label>
						<select
					...
					<div>
						...
						<p class="rounded-lg border border-white/10 bg-white/[0.01] px-4 py-3 text-sm text-white/30 select-none">
				...
				<div>
					...
					<textarea
						...
						class="w-full rounded-lg border border-white/10 bg-[#151515] px-4 py-3 text-sm text-white/70 outline-none focus:border-[#FFC700]/50 placeholder-white/20"
						required
					></textarea>
				</div>
