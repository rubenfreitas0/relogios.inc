...
<template>
	<div>
		...
		<div v-else-if="store.error" class="rounded-xl border border-red-500/20 bg-red-500/5 p-6 text-center">
			...
			<button @click="goBack" class="mt-4 text-xs font-bold uppercase tracking-wider text-[#FFC700] hover:text-yellow-300">
		...
		<div v-else-if="store.currentOrder">
			<!-- Back + Header -->
			<button @click="goBack" class="mb-6 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-white/40 transition-colors hover:text-[#FFC700]">
			...
			<div v-if="currentStepIndex >= 0" class="mb-10 rounded-xl border border-white/10 bg-white/[0.03] p-6">
				<div class="flex items-center justify-between">
					...
					>
						<div
							...
							"
						>
							<svg v-if="i < currentStepIndex" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" /></svg>
							<span v-else>{{ i + 1 }}</span>
						</div>
						<p class="mt-2 text-center text-[0.6rem] font-bold uppercase tracking-wider" :class="i <= currentStepIndex ? 'text-[#FFC700]' : 'text-white/20'">
							{{ timelineLabels[step] }}
						</p>
					</div>
				</div>
			...
			<div class="mb-6 rounded-xl border border-white/10 bg-white/[0.03] p-6">
				<h3 class="mb-4 text-[0.65rem] font-bold uppercase tracking-[0.15em] text-[#FFC700]">Artigos</h3>
				<div class="space-y-4">
					<div
					...
					>
						...
						<div class="min-w-0 flex-1">
							<p class="truncate text-sm font-semibold text-white">{{ item.product_name }}</p>
							<p class="text-xs text-white/30">Qtd: {{ item.quantity }} × €{{ formatPrice(item.unit_price) }}</p>
						</div>
						<p class="text-sm font-bold text-white">€{{ formatPrice(item.item_total) }}</p>
					</div>
				</div>
			</div>
			...
			<div class="grid gap-6 md:grid-cols-2">
				...
				<div class="rounded-xl border border-white/10 bg-white/[0.03] p-6">
					...
					<div class="space-y-2 text-sm">
						<div class="flex justify-between"><span class="text-white/40">Subtotal</span><span class="text-white/70">€{{ formatPrice(store.currentOrder.subtotal) }}</span></div>
						<div class="flex justify-between"><span class="text-white/40">Envio</span><span class="text-white/70">€{{ formatPrice(store.currentOrder.shipping_cost) }}</span></div>
						<div class="flex justify-between"><span class="text-white/40">IVA ({{ store.currentOrder.tax_rate }}%)</span><span class="text-white/70">€{{ formatPrice(store.currentOrder.tax_amount) }}</span></div>
						<div class="flex justify-between border-t border-white/10 pt-2"><span class="font-bold text-white">Total</span><span class="font-bold text-[#FFC700]">€{{ formatPrice(store.currentOrder.total) }}</span></div>
					</div>
				</div>

				<!-- Morada de envio -->
				<div class="rounded-xl border border-white/10 bg-white/[0.03] p-6">
					<h3 class="mb-4 text-[0.65rem] font-bold uppercase tracking-[0.15em] text-[#FFC700]">Envio</h3>
					<div class="text-sm text-white/60 space-y-1">
						<p class="font-semibold text-white">{{ store.currentOrder.customer.firstname }} {{ store.currentOrder.customer.lastname }}</p>
						<p>{{ store.currentOrder.shipping_address.address_line1 }}</p>
						<p v-if="store.currentOrder.shipping_address.address_line2">{{ store.currentOrder.shipping_address.address_line2 }}</p>
						<p>{{ store.currentOrder.shipping_address.postal_code }} {{ store.currentOrder.shipping_address.city }}</p>
						<p>{{ store.currentOrder.shipping_address.country }}</p>
						<p v-if="store.currentOrder.shipping_method" class="mt-3 text-xs text-white/30">
							{{ store.currentOrder.shipping_method.carrier }} — {{ store.currentOrder.shipping_method.name }}
						</p>
