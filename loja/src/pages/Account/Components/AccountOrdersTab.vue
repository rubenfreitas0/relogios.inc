...
<template>
	<div>
		...
		<!-- Empty -->
		<div
			v-else-if="store.orders.length === 0 && !store.isLoading"
		...
		<div v-else class="space-y-3">
			...
			>
				...
				<div class="flex items-center gap-4 md:gap-6">
					...
					<p class="min-w-[5rem] text-right text-sm font-bold text-white">
						€{{ formatPrice(order.total) }}
					</p>
					<svg
						class="h-4 w-4 text-white/20 transition-transform duration-200 group-hover:translate-x-1 group-hover:text-[#FFC700]"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
			...
			<!-- Pagination -->
			<div
				v-if="store.ordersPagination.last_page > 1"
